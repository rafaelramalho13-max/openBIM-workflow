...
def auto_generate_bep_excel():
    import subprocess
    import sys
    current_dir = os.path.dirname(os.path.abspath(__file__))
    script_path = os.path.join(current_dir, "requisitos", "gerar_anexo_bep.py")
...
with tab1:
    if validation_data:
        ...
        st.info("💡 Bem-vindo! Selecione um modelo IFC e um arquivo IDS no painel à esquerda e clique em **Executar Validação openBIM** para começar.")
        
        # Show local models ready to audit
        st.markdown("### Modelos Prontos na Pasta do Projeto:")
        local_files_list = []
        for name, path in local_ifc_names.items():
            sz = os.path.getsize(path) / (1024 * 1024)
            local_files_list.append({"Nome do Arquivo": name, "Tamanho": f"{sz:.1f} MB"})
        if local_files_list:
            st.table(local_files_list)
        else:
            st.warning("Nenhum arquivo IFC encontrado na pasta 'modelos/'. Adicione arquivos para começar.")
...
with tab4:
    ...
    pir_path = os.path.join(current_dir, "requisitos", "PIR.md")
    
    # Auto-generate if missing
    if not os.path.exists(bep_excel_path):
        with st.spinner("Gerando arquivo Excel do BEP automaticamente..."):
            auto_generate_bep_excel()
            
    # Download excel BEP button
    if os.path.exists(bep_excel_path):
        ...
        st.warning("O arquivo Anexo_BEP.xlsx não pôde ser gerado automaticamente. Verifique se o script `requisitos/gerar_anexo_bep.py` existe.")
    ...
    if os.path.exists(pir_path):
        ...
        st.info("O documento requisitos/PIR.md não foi localizado no workspace.")
